...
/// The BIP39 English wordlist — the same 2048 words crypto wallets use for
/// recovery phrases.
///
/// Taken verbatim from the Bitcoin BIPs repository, SHA-256
/// 2f5eed53a4727b4bf8880d8f3f199efc90e58503646d9ff8eff3a2ed3b24dbda. Do not
/// edit, reorder, or "tidy" it: the position of every word is what encodes the
/// key, so changing a single entry silently breaks every phrase ever issued.
///
/// The list is chosen, not arbitrary. Every word is unique within its first
/// four letters, which is what lets the entry field finish a word from a few
/// keystrokes, and near-homophones were deliberately kept out so a phrase read
/// aloud can be written down correctly.
enum BIP39Wordlist {
    /// Stored as one literal and split on first use. A 2048-element array
    /// literal is technically the same data, but it makes the type checker
    /// crawl through the whole file on every build.
    static let words: [String] = raw
        .split(whereSeparator: \.isWhitespace)
        .map(String.init)

    /// Position of a word, or nil if it isn't in the list. Backed by a
    /// dictionary because unlocking checks every word in a phrase and a linear
    /// scan of 2048 entries per word is wasteful for no reason.
    static func index(of word: String) -> Int? { lookup[word] }

    private static let lookup: [String: Int] = Dictionary(
        uniqueKeysWithValues: words.enumerated().map { ($1, $0) }
    )
